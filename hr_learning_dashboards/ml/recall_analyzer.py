...
class RecallAnalyzer:
    ...
    def prepare_features(self, df):
        ...
        # Создаем lag-фичи (если их нет в запросе)
        ...
        feature_cols = [
            ...
            'delta_days',
            'log_delta',
            # 'response_time',  # временно уберём, если подозреваешь
            # 'response_time_ratio',
            # 'prev_outcome',    # может давать утечку
            'prev_response_time',
            'prev_delta',
    ...
    def train(self, df=None):
        ...
        self.check_data_leakage()  # ← добавить эту строку
        ...
        # ИСПРАВЛЕНО: early_stopping_rounds передаем в fit, а не в конструктор
        self.model = xgb.XGBClassifier(
            n_estimators=200,
            max_depth=6,
            learning_rate=0.1,
            ...
            random_state=42,
            eval_metric='logloss',
            early_stopping_rounds=20,  # ← перенесли сюда
    ...
    def check_data_leakage(self):
        """Проверяет, нет ли утечки данных"""
        ...
        # Смотрим корреляцию каждой фичи с target
        correlations = []
    ...
    def shap_analysis(self, sample_size=500, save_path='ml/shap_summary.png'):
        ...
        # Берем выборку из тестовых данных
        ...
        # Зависимость вероятности от топ-фактора
        if top_feature in self.df_clean.columns:
            plt.figure(figsize=(10, 5))

            # Биннинг для непрерывных признаков
    ...
    def get_insights(self):
        """Генерирует бизнес-выводы на основе анализа"""
        if self.model is None:
        ...
        for i, (feat, idx) in enumerate(zip(top_3_features, top_3_idx)):
            ...
            # Интерпретация в зависимости от признака
